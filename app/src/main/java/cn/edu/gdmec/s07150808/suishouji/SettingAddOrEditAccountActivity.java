	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		findViewById(R.id.save_btn).setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				...
				if (TextUtils.isEmpty(account_name)) {
					Toast.makeText(getApplicationContext(), getString(R.string.error_user_empty), Toast.LENGTH_SHORT).show();
					return;
				}
				
				if (TextUtils.isEmpty(account_balance_s)) {
					Toast.makeText(getApplicationContext(), getString(R.string.error_balance_empty), Toast.LENGTH_SHORT).show();
					return;
				}
				...
				try {
					...
					Toast.makeText(getApplicationContext(), getString(R.string.error_balance_number), Toast.LENGTH_SHORT).show();
					return;
				}
				...
				if (mode == NEW_MODE) {
					if (CommonData.getInstance().existAccount(account_name)) {
						Toast.makeText(getApplicationContext(), getString(R.string.error_user_there), Toast.LENGTH_SHORT).show();
						return;
					}
		...
		case EDIT_MODE:
			...
			if (id >= 0) {
				...
				Toast.makeText(this, getString(R.string.error_system), Toast.LENGTH_SHORT).show();
				finish();
			}
		...
		if (account_data != null) {
			...
			if (mode == EDIT_MODE) {
				...
				CommonData.AccountCategoryData subcat = commondata.accountsubcategory.get(account_data.category);
